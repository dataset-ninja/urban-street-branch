...
def convert_and_upload_supervisely_project(
    ...
    classes_path = os.path.join("branch","VOCdevkit","VOC2012","class_names.txt")
    images_path = os.path.join("branch","VOCdevkit","VOC2012","JPEGImages")
    masks_path = os.path.join("branch","VOCdevkit","VOC2012","SegmentationClass")
    split_folder = os.path.join("branch","VOCdevkit","VOC2012","ImageSets","Segmentation")
    images_ext = ".jpg"
    masks_ext = ".png"
    ...
    def create_ann(image_path):
        ...
        class_name = image_name.split("_branch")[0]
    ...
    class_name_to_mask_name = {
        "branch of Albizia julibrissin": "Albizia julibrissin",
        "branch of Flowering cherry": "Flowering cherry",
        "branch of Ginkgo biloba": "Ginkgo biloba",
        "branch of Koelreuteria paniculata": "Koelreuteria paniculata",
        "branch of Lagerstroemia indica": "Lagerstroemia indica",
        "branch of Liriodendron chinense": "Liriodendron chinense",
        "branch of Magnolia liliflora Desr": "Magnolia liliflora Desr",
        "branch of Platanus": "Platanus",
        "branch of Prunus cerasifera f. atropurpurea": "Prunus cerasifera f. atropurpurea",
        "branch of Salix babylonica": "Salix babylonica",
        "branch of Styphnolobium japonicum": "Styphnolobium japonicum",
        "branch of Triadica sebifera": "Triadica sebifera",
        "branch of Zelkova serrata": "Zelkova serrata",
    }
        ...
        for idx, class_name in enumerate(content):
            if idx != 0:
                mask_path = os.path.join(
                    masks_path,
                    class_name_to_mask_name.get(class_name, class_name) + "_branch_1 (1).png",
                )
                mask_np = sly.imaging.image.read(mask_path)
                unique_colors = get_unique_colors(mask_np)
                class_name_lower = class_name.lower()
    ...
    for split_file in os.listdir(split_folder):
        ...
        with open(split_path) as f:
            ...
            for im_name in content:
                if len(im_name) > 0:
                    if im_name == "Albizia julibrissin_branch_1 (91)":
                        im_name = "Albizia julibrissin_branch_1 (91) "
                    images_names.append(im_name + images_ext)
